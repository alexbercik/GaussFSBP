...
function _format_py_tuple(t::Tuple, num_digits::Int)
    parts = [_format_py_float(x, num_digits) for x in t]
    return "(" * join(parts, ", ") * ")"
end
...
function print_fsbp_operator_python(io::IO, op::FSBPOperator; num_digits::Int=5)
    println(io, "interval=", _format_py_tuple(op.interval, num_digits), ",")
